...
class Turtlebot3Drive:
    def __init__(self):
        ...
        self.escape_range_180 = 60.0 * DEG2RAD
        self.escape_range_lim = 45.0 * DEG2RAD
        self.check_forward_dist = 0.35
    ...
    def control_loop(self):
        ...
        if self.turtlebot3_state_num == GET_TB3_DIRECTION:
            ...
            if self.scan_data[CENTER] < self.check_forward_dist:
                ...
                elif self.scan_data[LEFT] < self.scan_data[RIGHT]:
                    self.prev_tb3_pose = self.tb3_pose
                    self.turtlebot3_state_num = TB3_RIGHT_TURN_LIM
                    print("right turn2")

                elif self.scan_data[RIGHT] < self.scan_data[LEFT]:
                    self.prev_tb3_pose = self.tb3_pose
                    self.turtlebot3_state_num = TB3_LEFT_TURN_LIM
                    print("left turn2")
        ...
        elif self.turtlebot3_state_num == TB3_LEFT_TURN_LIM:
            if fabs(self.prev_tb3_pose - self.tb3_pose) >= self.escape_range_lim:
                print("left turn finish")
                self.turtlebot3_state_num = GET_TB3_DIRECTION
            else:
                print("left turn executed")
        ...
        elif self.turtlebot3_state_num == TB3_RIGHT_TURN_LIM:
            if fabs(self.prev_tb3_pose - self.tb3_pose) >= self.escape_range_lim:
                print("right turn finish")
                self.turtlebot3_state_num = GET_TB3_DIRECTION
            else:
